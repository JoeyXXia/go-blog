  ...
  <div class="web-footer">
    <div class="container">
      ...
      <div class="footer-center">
        ...
        <div class="create-at">
          <el-text
            >建站日期：{{ websiteStore.state.websiteInfo.created_at }} 网站已运行：{{
              elapsedTime
            }}</el-text
          >
        </div>
...
import { type FooterLink, websiteFooterLink } from '@/services/modules/website'
...
function updateElapsedTime() {
  const creationDate = websiteStore.state.websiteInfo.created_at
  if (creationDate) {
    const creationTimestamp = new Date(creationDate).getTime()
    const currentTimestamp = new Date().getTime()

    const totalDays = (currentTimestamp - creationTimestamp) / 1000 / (60 * 60 * 24)
    const daysPassed = Math.floor(totalDays)
    const hoursRemaining = Math.floor((totalDays - daysPassed) * 24)
    const minutesRemaining = Math.floor((totalDays - daysPassed - hoursRemaining / 24) * 24 * 60)
    const secondsRemaining = Math.floor(
      (totalDays - daysPassed - hoursRemaining / 24 - minutesRemaining / 24 / 60) * 24 * 60 * 60,
    )
